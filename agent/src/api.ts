...
export async function buildServer() {
  ...
  server.post("/api/shock", async () => ({ ok: true, state: applyShock() }));
  ...
  server.post("/api/reset", async (request) => {
    ...
    return { ok: true, state: resetState({ hard }) };
  });

  server.post("/api/autopilot", async (request) => ({
    ok: true,
    state: setAutopilot(boolBody(request.body, "on")),
  }));

  server.post("/api/pause", async (request) => ({
    ok: true,
    state: setPaused(boolBody(request.body, "paused")),
  }));

  server.post("/api/profile", async (request) => ({
    ok: true,
    state: setProfile(profileBody(request.body)),
  }));

  server.post("/api/autohedge", async (request) => ({
    ok: true,
    state: setAutoHedge(boolBody(request.body, "on")),
  }));
  ...
  server.post("/api/policy/sync-profile", async (request) => {
    ...
    return { ok: true, sync: result, state: getState() };
  });
